...
class UCTreePolicy(object):
    ...
    def __call__(self, tree, environment):
        if not tree.action_statistics:
            tree.action_statistics = {a: (np.zeros(environment.num_players), 0) for a in environment.legal_actions()}

        N_s = tree.node_statistics or 1
        current_player = environment.current_player()
        uct_scores = {a: q[current_player] + self.C * np.sqrt(np.log(N_s) / n_a)
    ...
    def optimal_action(self, tree, environment):
        current_player = environment.current_player()
        qvalues = {a: q[current_player] for a, (q, n_a) in tree.action_statistics.items()}
